...
# 댓글더보기를 누른 상태의 url이여야 동작


# csv 파일로 저장
def write_csv(no, content, date): # 긁어온 댓글 순서, 내용, 작성시간을 csv파일로 저장
    with open('5-3.csv', 'a', encoding = 'utf-8', newline='', errors= 'replace') as file:
        # sample.csv 라는 파일을 생성 'a' : 추가모드로 파일을 오픈
        w = csv.writer(file) # sample.csv 에 입력
        w.writerow([no, content, date]) # 수집한 no, content, data 값을 입력

# 네이버 뉴스 링크 입력
url = 'https://news.naver.com/main/read.nhn?mode=LSD&mid=sec&sid1=100&oid=032&aid=0002776931&viewType=pc&m_view=1&m_url=%2Fcomment%2Fall.nhn%3Fgno%3Dnews032%2C0002776931'
# url에 접근
...
try:
    ...
    # 뜰 때까지 대기
    try:
        WebDriverWait(wd, 5).until(EC.presence_of_element_located((By.CLASS_NAME, "u_cbox_contents"))) # 웹 페이지가 뜰 때까지 대기. 근데 왜 옵션이 이렇게 많이 붙었을까?
    ...
    while True:
        ...
        # 댓글 개수가 늘어났으면 탈출
        for i in range(0, 20):
            # xpath 라는 변수 안에 네이버 댓글 중 수집할 내용이 들어있는 클래스만 접근!
            # //: 현재 node로 부터 문서상의 모든 node 조회, @: 현재 node의 속성 선택
            xpath = '//ul[@class="u_cbox_list"]/li[' + str(temp_contents_len) + ']/div/div/div/span[@class="u_cbox_contents"]'
            # xpath 안에 temp_contents_len 번째의 댓글의 경로 입력
            # cbox_list : 댓글의 전체 클래스, cbox_contents : 실제 댓글 내용
            xpath1 = '//ul[@class="u_cbox_list"]/li[' + str(temp_contents_len) + ']/div/div/div/span[@class="u_cbox_date"]'
            # xpath 안에 temp_contents_len 번째의 댓글 작성 시간의 경로 입력
            # u_cbox_date : 댓글 작성 시간의 정보를 담고 있는 클래스!
            print(temp_contents_len) # 댓글 순서
            temp_contents_len += 1 # 댓글 순서 +1

            try:
                WebDriverWait(wd, 0.05).until(EC.presence_of_element_located((By.XPATH, xpath)))

                # 정상 댓글이면 정상 출력
                content = wd.find_element_by_xpath(xpath).text
                # content값에 위에서 설정했던 경로들을 대입!
        ...
        # 댓글 더보기 클릭 (옛날 기사는 엘리먼트이름이 다름)
        wd.find_element_by_xpath('//*[@class="u_cbox_page_more"]').click()
